# Earliest Finish Time for Land and Water Rides I - Rust Solution
...
```rust
fn best_follow_up(first_finish: i32, rides: &[Ride], prefix: &[i32], suffix: &[i32]) -> i32 {
    let idx = rides.partition_point(|r| r.start <= first_finish);
    let mut ans = i32::MAX;

    if idx > 0 {
        ans = ans.min(first_finish + prefix[idx - 1]);
    }
    if idx < rides.len() {
        ans = ans.min(suffix[idx]);
    }
...
Both implementations are included in `main.rs` and verified against the same test cases.
